  ...
  const handleImportPreview = (file) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const header = rows[0];
        const expectedHeader = ["Mã sách", "Tên sách", "Số lượng nhập", "Giá nhập"];
        if (
          !header ||
          expectedHeader.some(
            (h, i) => (header[i] || "").trim().toLowerCase() !== h.toLowerCase()
          )
        ) {
        ...
        if (rows.length <= 1) {
          messageApi.warning("⚠️ File không có dữ liệu để import");
          return;
        }
