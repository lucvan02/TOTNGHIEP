...
export default function Statistics() {
  ...
  const load = async () => {
    try {
      ...
      const [kpiRes, revRes, ordRes, topBooksRes] = await Promise.all([
        statsApi.kpis(from, to),
        statsApi.revenueDaily(from, to),
        statsApi.ordersDaily(from, to),
        statsApi.topBooks(from, to, 10),
      ...
      setRev(
        (revRes.data.data || []).map((x) => ({
          date: x.date,
          value: x.value,
        }))
      );
      setOrd(
        (ordRes.data.data || []).map((x) => ({
          date: x.date,
          value: x.value,
        }))
      );
  ...
  const totalBooks = useMemo(() => kpi?.totalBooks ?? 0, [kpi]);
  const totalOrders = useMemo(() => kpi?.totalOrders ?? 0, [kpi]);
  const completedOrders = useMemo(() => kpi?.completedOrders ?? 0, [kpi]);
  const totalRevenue = useMemo(() => kpi?.revenue ?? 0, [kpi]);
  ...
  return (
    <Space direction="vertical" size="large" style={{ width: "100%" }}>
      ...
      <Row gutter={16}>
        ...
        <Col xs={24} md={6}>
          <Card loading={loading}>
            <Statistic title="Tổng doanh thu" value={money(totalRevenue)} />
          </Card>
        </Col>
      </Row>
      ...
      <Card title="🔥 Top sách bán chạy" loading={loading}>
        <Table
          ...
          columns={[
            ...
            {
              title: "Đã bán",
              dataIndex: "qty", // hoặc 'saleQuantity' nếu BE trả vậy
              align: "right",
              width: 120,
              sorter: (a, b) =>
                (a.qty || a.saleQuantity) - (b.qty || b.saleQuantity),
              render: (v, r) => v ?? r.saleQuantity ?? 0,
            },
            {
              title: "Doanh thu",
              dataIndex: "revenue",
              ...
              render: (v, r) =>
                money(v ?? r.totalRevenue ?? r.revenue ?? 0),
            },
          ]}
        />
      </Card>
