...
export default function OrderDetail() {
  ...
  const pmenuOpen = Boolean(anchorEl);
  const openPMenu = (e) => setAnchorEl(e.currentTarget);
  const closePMenu = () => setAnchorEl(null);
  ...
  const canSwitchPM = order?.status === "PENDING" && !order?.paymentStatus; // chỉ khi chưa thanh toán
  // const canPayOnline = order?.status === "PENDING"  && !order?.paymentStatus && order?.paymentMethod === "ONLINE";
  // //khi trang thai khác completed, cancelled và chưa thanh toán và phương thức là ONLINE
  const canPayOnline = order?.status !== "COMPLETED" && order?.status !== "CANCELLED" && !order?.paymentStatus && order?.paymentMethod === "ONLINE";//khi trang thai khác completed, cancelled và chưa thanh toán và phương thức là ONLINE
  ...
  const doCancel = async () => {
    ...
    try {
      ...
      setOrder((prev) => ({ ...prev, status: updated.status, cancelReason: updated.cancelReason, paymentStatus: updated.paymentStatus ?? prev.paymentStatus }));
  ...
  const switchPayment = async (method) => {
    try {
      closePMenu();
      setBusy(true);
      const res = await orderApi.changePaymentMethod(order.id, method); // {message,data:Order}
      setOrder(res.data.data);
      notify("success", `Đã chuyển phương thức sang ${method}`);
    } catch (e) {
      console.error(e);
      notify("error", e?.response?.data?.message || "Đổi phương thức thất bại");
    } finally {
      setBusy(false);
    }
  };
  ...
  // Go pay online
  const goPayOnline = async () => {
    try {
      setBusy(true);
      // đảm bảo phương thức đang ONLINE
      if (order.paymentMethod !== "ONLINE") {
        await switchPayment("ONLINE");
      }
      const res = await orderApi.payOnline(order.id); // { paymentUrl }
      const url = res.data?.data?.paymentUrl || res.data?.data?.url || res.data?.data;
      if (!url) throw new Error("Không nhận được paymentUrl");
      window.location.href = url;
    } catch (e) {
      console.error(e);
      notify("error", e?.response?.data?.message || e.message || "Không tạo được link thanh toán");
    } finally {
      setBusy(false);
    }
  };
        ...
        <Box display="flex" gap={1} alignItems="center">
          ...
          {canSwitchPM && (
            <>
              ...
              <Menu anchorEl={anchorEl} open={pmenuOpen} onClose={closePMenu}>
                <MenuItem onClick={() => switchPayment("COD")}>Chuyển sang COD</MenuItem>
                <MenuItem onClick={() => switchPayment("ONLINE")}>Chuyển sang ONLINE</MenuItem>
              </Menu>
            </>
          )}
          ...
          {canCancel && (
            <Button color="error" variant="outlined" onClick={openCancel} disabled={busy}>
              Hủy đơn
            </Button>
          )}
