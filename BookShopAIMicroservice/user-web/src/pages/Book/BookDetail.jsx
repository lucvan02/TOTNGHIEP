...
export default function BookDetail() {
  ...
  const count = useMemo(
    () => Number(book?.ratingCount ?? book?.reviewCount ?? 0),
    [book]
  );
  ...
  // Nếu sách có trạng thái ẩn (status=0) thì chỉ hiển thị thông báo
  if (book.status === 0) {
    return (
      ...
      <Card sx={{ display: "flex", flexDirection: { xs: "column", md: "row" }, p: 4, boxShadow: 4, borderRadius: 3, backgroundColor: "#fff" }}>
        ...
        <Box flex={1} ml={{ md: 4 }} mt={{ xs: 3, md: 0 }}>
          ...
          {book.status === 2 ? (
            ...
            <>
              <Typography variant="subtitle1" color="text.secondary" mb={2}>
                ...
                <br />
                Đã bán: {book.saleQuantity?? 0}  {book.stock === 0 ? "" : `| Còn lại: ${book.stock}`}
              </Typography>
          ...
          <Box>
            <Typography variant="body1" fontWeight="bold">Thể loại:</Typography>
            {book.categories?.map((c) => (
              //hover có hiệu ứng pointer
              <Tag color="geekblue" key={c.id} style={{ marginTop: 5, cursor: "pointer" }} onClick={() => navigate(`/category/${c.id}`)}>
                {c.name}
              </Tag>
            ))}
          </Box>
          ...
          <Box>
            <Typography variant="body1" fontWeight="bold">Tác giả:</Typography>
            {book.authors?.map((a) => (
              <Tag color="purple" key={a.id} style={{ marginTop: 5, cursor: "pointer" }} onClick={() => navigate(`/author/${a.id}`)}>
                {a.name}
              </Tag>
            ))}
          </Box>
          ...
          <Box>
            <Typography variant="body1" fontWeight="bold">Nhà xuất bản:</Typography>
            {/* {book.publisher && <Tag color="green" style={{ marginTop: 5, cursor: "pointer" }} onClick={() => navigate(`/publisher/${book.publisher.id}`)}>{book.publisher.name}</Tag>} */}
            {book.publisher && <Tag color="green" style={{ marginTop: 5, cursor: "pointer" }} onClick={() => navigate(`/publisher/${book.publisher.id}`)}>{book.publisher.name}</Tag>}
          </Box>
        </Box>
      </Card>
